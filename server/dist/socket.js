...
import { publicAttack } from "./game/GameState.js";
...
export function attachSocketHandlers(io, rooms) {
    ...
    const ensureLoop = (room) => {
        ...
        const handle = setInterval(() => {
            ...
            for (const combat of result.combats)
                io.to(room.id).emit("workerArrived", combat);
            ...
            for (const playerId of result.monsterImpacts)
                io.to(room.id).emit("monsterImpact", { playerId, damage: GAME.monsterDamage });
            if (result.ended)
                io.to(room.id).emit("gameEnded", result.ended);
            const now = Date.now();
    ...
    const joinSocketRoom = (socket, room, playerId) => {
        const data = socket.data;
        data.playerId = playerId;
        data.roomId = room.id;
        socket.join(room.id);
    };
    const safe = (socket, fn) => {
        try {
            fn();
        }
        catch (error) {
    ...
    io.on("connection", (socket) => {
        socket.on("createRoom", (payload) => safe(socket, () => {
            if (!rateLimit.allow(socket.id, "createRoom", 5, 10_000))
                throw new Error("Too many room requests.");
        ...
        socket.on("joinRoom", (payload) => safe(socket, () => {
            if (!rateLimit.allow(socket.id, "joinRoom", 10, 10_000))
                throw new Error("Too many join requests.");
        ...
        socket.on("attackOffice", (payload) => safe(socket, () => {
            if (!rateLimit.allow(socket.id, "attackOffice", 20, 1_000))
                throw new Error("Attacking too quickly.");
            const data = socket.data;
            if (!data.roomId || !data.playerId)
                throw new Error("Not in a room.");
